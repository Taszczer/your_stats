...
export default function SideBar() {
  ...
  if (data) {
    ...
    lastLogOff == "Invalid Date" && (lastLogOff = "Hidden");
    ...
    return (
      <div className=" flex flex-col items-center min-h-[320px] h-[320px] md:h-screen w-screen md:min-w-[220px] md:w-[260px] lg:min-w-[260px] lg:w-[320px] xl:min-w-[300px] xl:w-[450px] rounded-bl-3xl rounded-br-3xl md:rounded-bl-[0px] md:rounded-tr-[24px] md:rounded-br-[24px] border-b-4 md:border-b-0 md:border-r-4 border-black bg-[#CC9614] relative overflow-hidden">
        <div
          className="h-full absolute top-4 left-4 lg:top-7 lg:left-7 cursor-pointer"
          onClick={() => router.push("/")}
        >
          <span className="material-symbols-outlined text-xl scale-[1.333336] lg:scale-[2] text-white">
            arrow_circle_left{" "}
          </span>
        </div>
        <div className="flex flex-col gap-4 lg:gap-7 items-center h-full w-full p-3 pt-9 md:p-3 lg:p-6">
          <div className="flex flex-row md:flex-col text-white text-center md:items-center gap-4 lg:gap-5">
            <div className="relative w-[80px] h-[80px] md:w-[120px] md:h-[120px] lg:w-[152px] lg:h-[152px] xl:w-[184px] xl:h-[184px]">
              <Image
                src={user.avatarfull}
                alt="profile_pic"
                fill
            ...
            <div className="flex flex-col items-start md:items-center justify-center">
              <div className="text-start md:text-center">
                <h1 className="text-xl xl:text-2xl font-['Angkor']">
                  {user.personaname}
                </h1>
                <p className="text-base xl:text-base">#{user.steamid}</p>
              </div>

              <h1 className="font-bold text-base text-center xl:text-xl">
                Status:{" "}
                <span className="text-black font-['Madimi_One'] ml-2">
                  {personaStateLabel}
                </span>
              </h1>
            </div>
          ...
          <div className="flex flex-col gap-2 text-base text-white font-medium text-center">
            ...
            <div className="flex flex-row items-center justify-center md:flex-col">
              <div className="flex flex-col gap-2 md:gap-1 xl:gap-0 max-w-[40%] md:max-w-full">
                <h2 className="text-base xl:text-xl md:font-semibold xl:font-medium">
                  {countryName}
                </h2>
                <p>
                  Created in:{" "}
                  <span className="text-black font-['Madimi_One'] ml-1">
                    {dateOfCreation}
                  </span>
                </p>
              </div>

              <div className="flex flex-col-reverse gap-2 md:gap-1 xl:gap-0 xl:flex-col max-w-[40%] md:max-w-full">
                <p>
                  Last log off:{" "}
                  <span className="text-black font-['Madimi_One'] ml-1">
                    {lastLogOff}
                ...
                {user.gameextrainfo ? (
                  <>
                    <p>
                      ...
                      <span className="text-black ml-1 font-['Madimi_One']">
                        {user.gameextrainfo}
                      </span>
                    </p>
                  </>
