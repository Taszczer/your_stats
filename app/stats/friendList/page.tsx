...
export default function FriendList(userId:any) {
    
    const { data, isPending, isError, error } = useQuery<userFriendsResponse>({
        queryKey: ['FriendList'],
        queryFn: async () => {
            const res = await axios.get("/api/steam/ISteamUser/GetFriendList/v0001/?key=297EF931003801AA8E111DF1E8FAC18B&steamid=76561198968190802&relationship=friend")
            return res.data
        }
    })
    ...
    return (
      <div className="flex flex-col gap-4 w-full items-center">
        ...
        <div className="w-full flex flex-col gap-2 overflow-y-auto scrollbar-hide max-h-[560px]">
          ...
          (
            <div key={friend.data?.steamid || key} className="flex flex-row items-center gap-6 px-4 py-3 bg-white rounded-xl border-[1px] border-r-4 border-b-4 border-black w-full">
              {friend.data?.avatar && (
                <Image
